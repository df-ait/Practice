...
#include "ekf_timer.hpp"
#include "ekf.h"
#include "kf.h"
#include "slidestd.hpp"
#include "slideweighted.hpp"
#include <algorithm>
...
public:
    ...
    Eigen::Matrix<double, 4, 1> getPose(double append_delay);//获取补偿延迟后的装甲板预测位姿
    Eigen::Matrix<double, 4, 1> getCenter(double append_delay);//获取目标中心点预测位姿（用于打击核心）

    void setMatrixQ(double, double, double, double, double, double, double, double, double);
    void setMatrixR(double, double, double, double);//设置运动模型的过程/观测噪声协方差
    ...
    void setRadiusRange(double r_min, double r_max) { r_min_ = r_min; r_max_ = r_max; }//动态调整装甲板半径约束
    void setArmorNum(int armor_num) { armor_num_ = armor_num; }//修改装甲板数量（影响角度权重计算）
    ...
    double getOmega() { return omega_model_.estimate_X[1];};//返回当前角速度估计值（rad/s）
