...
cv::Mat HIKimage;   // 存储相机捕获的当前帧图像
std::mutex HIKframemtx;//HIKdriver.hpp中声明外部变量的实际定义
...
bool PrintDeviceInfo(MV_CC_DEVICE_INFO* pstMVDevInfo)
{
    ...
    if (pstMVDevInfo->nTLayerType == MV_GIGE_DEVICE)//检测是否为GigE设备
    {
        //通过位操作提取IP的四个部分
    ...
    else if (pstMVDevInfo->nTLayerType == MV_USB_DEVICE)//检测是否为USB设备
    {
...
int HIKcamtask()
{
    int nRet = MV_OK;   // 存储SDK函数调用结果

    void* handle = NULL;    // 相机设备句柄
	unsigned char * pData = NULL;    // 原始图像数据缓冲区    
    unsigned char *pDataForRGB = NULL;  // RGB转换缓冲区
    ...
    do 
    {
        ...
        if (stDeviceList.nDeviceNum > 0)//检测到的设备数量大于0
        {
        ...
        // 开始取流
        // start grab image
        nRet = MV_CC_StartGrabbing(handle);//启动相机传感器采集，初始化DMA传输通道
        if (MV_OK != nRet)
        {
        ...
        cv::Mat frame, image;
        unsigned int nDataSize = stParam.nCurValue;//将相机单帧图像的数据大小赋值给nDataSize
        nRet = MV_CC_GetOneFrameTimeout(handle, pData, nDataSize, &stImageInfo, 1000);//该函数用于捕获单帧图像数据
        if (MV_OK != nRet)
        {
            break;
        }
        ...
        //设置相机gain（类似ISO）
        MV_CC_SetGain(handle, params.cam_gain);//设置相机增益
        MV_CC_SetExposureTime(handle, params.cam_exptime);//设置相机曝光时间us
        MV_CC_SetFrameRate(handle, params.framerate);//设置相机帧率
        ...
        while(1){
            ...
            if (nRet == MV_OK)
            {
                ...
                cv::cvtColor(frame, image, cv::COLOR_RGB2BGR); //RGB转BGR​​
        ...
        // 停止取流
        // end grab image
        nRet = MV_CC_StopGrabbing(handle);//停止图像流采集，避免资源泄漏
        if (MV_OK != nRet)
        {
        ...
        // 销毁句柄
        // destroy handle
        nRet = MV_CC_DestroyHandle(handle);//释放SDK资源
        if (MV_OK != nRet)
        {
