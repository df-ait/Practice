...
cv::Mat yolo_kpt::letter_box(cv::Mat &src, int h, int w, std::vector<float> &padd) {
    ...
    int inside_w = round(in_w * r);//四舍五入到整数
...
cv::Rect yolo_kpt::scale_box(cv::Rect box, std::vector<float> &padd, float raw_w, float raw_h) {
    cv::Rect scaled_box;
    scaled_box.width = box.width / padd[2];// padd[0]为水平填充，padd[2]为缩放比
...
void yolo_kpt::drawPred(int classId, float conf, cv::Rect box, std::vector<cv::Point2f> point, cv::Mat &frame,
    ...
    float x0 = box.x;
    float y0 = box.y;//左上角坐标
    float x1 = box.x + box.width;
    float y1 = box.y + box.height;//右下角坐标
    ...
    std::string label = cv::format("%.2f", conf);//置信度保留两位小数
    if (!classes.empty()) {
...
void yolo_kpt::generate_proposals(int stride, const float *feat, std::vector<Object> &objects) { //后处理部分，重做检测层
    ...
    int feat_h = IMG_SIZE / stride;//特征图的宽高
#if IMG_SIZE == 640     //w1,h1,w2,h2,w3,h3,w4,h4,w5,h5,w6,h6
    ...
    for (int anchor = 0; anchor < ANCHOR; anchor++) { //对每个anchor进行便利
        for (int i = 0; i < feat_h; i++) { // self.grid[i][..., 0:1]
            for (int j = 0; j < feat_w; j++) { // self.grid[i][..., 1:2]
                ...
                //中心点坐标：(sigmoid(x)*2 -0.5 + grid_x) * stride
                //宽高：(sigmoid(w)*2)^2 * anchor_w
...
std::vector<yolo_kpt::Object> yolo_kpt::work(cv::Mat src_img) {
    ...
    //将HWC格式的OpenCV图像转为​​CHW​​格式的张量，像素值从[0,255]缩放到[0,1]
    ...
    /*---------------------推理----------------------*/
    timer.begin();
    infer_request.set_input_tensor(input_tensor1);//将预处理后的图像数据（input_tensor1）绑定到推理请求
//    infer_request.infer(); //推理并获得三个提取头
    infer_request.start_async();//异步推理
    infer_request.wait();//等待推理完成
    auto output_tensor_p8 = infer_request.get_output_tensor(0);
    const float *result_p8 = output_tensor_p8.data<const float>();//获取输出指针
    auto output_tensor_p16 = infer_request.get_output_tensor(1);//stride = 16
    const float *result_p16 = output_tensor_p16.data<const float>();
    auto output_tensor_p32 = infer_request.get_output_tensor(2);//stride = 32
