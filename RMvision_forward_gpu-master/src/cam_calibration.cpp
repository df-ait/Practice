...
void writeCameraParametersToJson(const cv::Mat& cameraMatrix, const cv::Mat& distCoeffs, const std::string& filename);//将OpenCV格式的相机参数保存到文件
void readCameraParametersFromYaml(const std::string& filename, cv::Mat& cameraMatrix, cv::Mat& distCoeffs);//从YAML文件加载相机参数到OpenCV矩阵
int calibration_main();//执行完整的相机标定流程
...
void camrea_calibtation::init(){
    /*************** cam settings ****************/
    camrea.cam.open(camrea.cam_id, cv::CAP_V4L2);//打开相机设备
